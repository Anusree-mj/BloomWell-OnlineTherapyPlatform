...
// Define the types for therapistDetails and ongoingActivity
interface TherapistDetails {
  _id: string;
  name: string;
}

interface OngoingActivityItem {
  _id: string;
  date: string;
  sessionStart?: string;
  sessionEnd?: string;
...
interface OngoingActivityComponentProps {
  therapistDetails: TherapistDetails | null;
  ongoingActivity: OngoingActivityItem[];
  isTable:Boolean
}

const OngoingActivityComponent: React.FC<OngoingActivityComponentProps> = ({ therapistDetails, ongoingActivity,isTable }) => {
  ...
  return (
    <Box sx={{ backgroundColor: '#325343', pb: 8 }}>
      {isTable? (
        ...
        <Box sx={{ display: 'flex', backgroundColor: '#325343', flexDirection: 'column', minHeight: '80vh', alignItems: 'center', justifyContent: 'center', pb: 8 }}>
          <DoSomethingComponent content="You haven't connected to any therapist yet!" buttonTitle="Let's Connect" url="/client/connection" />
        </Box>
      )}
    </Box>
  );
